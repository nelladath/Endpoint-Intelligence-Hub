...
def summarize_patch_compliance(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    applicable_total = counts["Compliant"] + counts["Non-Compliant"]
    success_ratio = counts["Compliant"] / applicable_total if applicable_total else 0.0
    return {
        "total": len(records),
        "success": counts["Compliant"],
        "failed": counts["Non-Compliant"],
        "stale": counts["Stale"],
        "endOfService": counts["OS End of Service"],
        "unknown": counts["Unknown"],
        "successRatio": round(success_ratio, 4),
        "healthStatus": classify_health(success_ratio),
    }
...
def build_report(
    ...
    return {
        ...
        "summary": {
            ...
            "patchCompliance": summarize_patch_compliance(patch_compliance),
        },
        "apps": apps,
