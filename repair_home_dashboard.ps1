...
function LastRefreshed() {
    if ($metrics.ContainsKey("Last Refreshed (UTC)")) {
        $raw = [string]$metrics["Last Refreshed (UTC)"].metricValue
        try { return ([datetimeoffset]::Parse($raw)).UtcDateTime.ToString("yyyy-MM-dd HH:mm") + " UTC" } catch { return $raw }
    }
...
<div style='font-size:18px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:#f8fafc;margin:0 0 4px 0'>Endpoint Intelligence Hub | Current scope: $(Metric 'Total Devices') Managed Devices</div>
<div style='font-size:12px;color:#94a3b8;margin:0 0 22px 0'>Last refreshed: $(LastRefreshed)</div>
